...
| **v0.18.2** | **shipped** | + `release.yml` GitHub Action: tag-triggered npm publish via OIDC trusted publisher (no long-lived tokens) + Sigstore provenance attestation. First package published via the automated pipeline, validating the trust chain end-to-end |
