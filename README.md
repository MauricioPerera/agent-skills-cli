...
| **v0.17.0** | **shipped** | + Rekor entry parsing + public-instance pinning + lookup by UUID (`parseRekorEntry`, `fetchRekorEntry`, `RekorEntry` types). Foundations for client-side Level 4 verification — **no crypto verification yet**. Real fixture committed (`tests/fixtures/rekor-entry-sample.json`) for offline testing. Spec patch v0.4.0: formalizes the Level 4 verification *contract* (§5.4) — what banks must do, not how |
| v0.18.0+ | planned | Phase 2 of Level 4: full verification (Merkle inclusion-proof math + Rekor checkpoint signature + Fulcio cert chain + identity matching against subscription's `expected_subject` / `expected_issuer`). Implementation likely via `@sigstore/verify` (audited official lib) — first dep posture change since project start, deserves explicit decision before merge |
